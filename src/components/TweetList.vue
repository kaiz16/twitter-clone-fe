...
export default {
  ...
  methods: {
    ...
    async likeTweet(tweetID) {
      ...
      const hasLiked = tweet.likes.find(
        (like) => like.likedBy.email === this.userStore.user.email
      );
      if (hasLiked) {
        // Unlike the tweet
        await this.tweetStore.deleteLike({
          tweetId: tweetID,
          likeId: hasLiked.id,
        });
        return;
      ...
      await this.tweetStore.addLike({
        tweetId: tweetID,
        likedBy: this.userStore.user,
      });
    },
  },
};
...
<template>
  <div
    class="hover:bg-black/[0.03] cursor-pointer p-4 border-b border-[#eff3f4]"
      ...
      <div class="flex-1">
        <div class="flex justify-between">
          <div class="flex items-center space-x-1 text-[15px]">
            ...
            <span class="text-[#536471]">@{{ tweet.createdBy.username }}</span>
        ...
        <div class="flex items-center space-x-16 mt-2 -ml-2">
          ...
          >
            <div
              class="group-hover:bg-[#1d9bf0]/10 group-hover:text-twitter rounded-full p-2 transition-all"
            >
              <ChatBubbleLeftIcon class="h-4 w-4" />
            </div>
            <span class="text-sm group-hover:text-twitter transition-all">{{
          ...
          >
            ...
            >
              <HeartIcon
                :class="[hasLiked(tweet) ? 'fill-[#f91880]' : '', 'h-4 w-4']"
              />
            </div>
            <span
              :class="[
          ...
          <div class="flex items-center space-x-2 group cursor-pointer">
            <div
              class="hover:bg-[#1d9bf0]/10 hover:text-twitter rounded-full p-2 transition-all"
            >
