...
export default {
  ...
  async mounted() {
    this.loading = true
    await this.tweetStore.getTweetById(this.$route.params.tweetID)
    this.loading = false
    this.tweet = this.tweetStore.tweets[0]
  },
  computed: {
    hasLiked() {
      return this.tweet.likes.find((like) => like.likedBy.email === this.userStore.user.email)
    },
  },
  methods: {
    async createReply() {
      await this.tweetStore.addReply(this.tweet.id, this.replyContent, this.userStore.user.id)
    ...
    <template #default>
      ...
      <div class="p-4 flex flex-col">
        <div class="flex flex-col space-y-3">
          ...
          <div>
            <span class="text-[#536471] text-[15px]">{{ formatDate(tweet.createdAt) }}</span>
          </div>
        </div>
        ...
        <div class="flex justify-evenly items-center py-1.5 my-3 border-y border-[#eff3f4]">
          ...
          <div class="flex items-center space-x-2 group cursor-pointer">
            <div :class="[hasLiked ? 'text-[#f91880]' : 'group-hover:text-[#f91880] text-[#536471]', 'group-hover:bg-[#f91880]/10  rounded-full p-2 transition-all']">
              <HeartIcon :class="[hasLiked ? 'fill-[#f91880]' : '', 'h-6 w-6 ']" />
            </div>
          </div>
        ...
        <div class="flex items-center space-x-3">
          ...
          <div class="flex-1 text-[#0f1419] text-base md:text-xl">
            <input v-model="replyContent" class="w-full outline-none placeholder-[#536471]" type="text" placeholder="Tweet your reply" />
          </div>

          <button @click="createReply" class="bg-twitter hover:bg-[#1a8cd8] px-4 py-2 rounded-full text-white font-bold text-sm">Reply</button>
        </div>
      </div>
